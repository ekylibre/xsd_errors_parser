module XmlErrorsParser
  ...
  module ErrorsRegex
    ...
    ERRORS_REGEX = {
      ...
      '1826' => /('(?<element>.*?)')\:\s\'(?<value>.*)\'.is.not.a.valid.value.*union.type.'(?<type>.*)'/,
      '1831' => /('(?<element>.*?)').*.a.length.of\s('(?<length>.*?)').*('(?<min_length>.*?)')/,
      '1824' => /('(?<element>.*?)'):\s('(?<value>.*?)').*.atomic.type\s('(?<type>.*?)')/,
      '1871' => /('(?<element>.*?)'):.Missing.child.*Expected.is.(\(\s(?<child>.*?)\s\))/
    }
